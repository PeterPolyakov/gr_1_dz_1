$(document).ready(function () {
	...
	for (let i = 0; i < 60; i++) {
		...
		document.body.appendChild(brick.getDiv());
	...
	function checkCollision(objectA, objectB) {
		if (objectA !== objectB) {
			...
			if (ball && brick) {
				...
				const brickMarginRight = parseInt(brick.getDiv().style.marginRight);
				const brickMarginLeft = parseInt(brick.getDiv().style.marginLeft);

				const touchInMainWidth = ballMiddle > brick.x && ballMiddle < brick.x + brick.width;

				const touchWothMargin = ballMiddle > brick.x + brickMarginLeft && ballMiddle < brick.x + brick.width + brickMarginRight;

				if ((touchInMainWidth || touchWothMargin) && ball.y === brick.y + brick.height + 2) {
					ball.directionY = ball.directionY * -1;
